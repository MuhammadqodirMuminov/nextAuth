...
const auth: AuthOptions = {
	providers: [
		GoogleProvider({
			clientId: '118873240337-54a20fkted3hskldi02fh31fk46rmq11.apps.googleusercontent.com',
			clientSecret: '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',
		}),
		FacebookProvider({
			clientId: '1554496255404456',
			clientSecret: 'c1fc8888a7579ec27bf917ed82a29009',
		}),
		CredentialsProvider({
			name: 'Credentials',
			credentials: {
		...
		async signIn({ account, user }) {
			if (user) {
				if (account?.provider === 'google') {
					...
					const { data: checkUser } = await axios.get(`https://tour-back.uz/api/auth/check-user?email=${email}`);
